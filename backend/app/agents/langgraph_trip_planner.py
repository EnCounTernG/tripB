...
class LangGraphTripPlanner:
    ...
    def _node_information_react(self, state: TripPlanningState) -> TripPlanningState:
        ...
        tool_access = request.tool_access or "mcp"
        ...
        for step in range(1, 6):
            ...
            start = time.time()
            tool_result = self._run_tool_call(tool_access, action_name, action_input)
        ...
        print(f"🧪 [ReAct] action/observation轨迹 (tool_access={tool_access}):")
        for item in traces:
    ...
    def _run_tool_call(self, tool_access: str, action_name: str, action_input: Dict[str, Any]) -> Dict[str, Any]:
        if tool_access == "direct_http":
            return self._execute_direct_http_tool(action_name, action_input)
        return self.tool_registry.execute_with_fallback(action_name, action_input)

    def _execute_direct_http_tool(self, action_name: str, action_input: Dict[str, Any]) -> Dict[str, Any]:
        """绕过ToolRegistry，直接调用高德HTTP API，便于排查MCP工具层问题。"""
        try:
            if action_name == "map.search_poi":
                raw = amap_text_search(
                    keywords=action_input.get("keywords", "景点"),
            ...
            if action_name == "map.weather":
                raw = amap_weather(city=action_input.get("city", ""))
                payload = json.loads(raw)
                return {
                    "ok": True,
                    "degraded": False,
            ...
            if action_name == "map.route":
                ...
                city = action_input.get("city", "")
                origin_name = action_input.get("origin", "")
                destination_name = action_input.get("destination", "")
                mode = action_input.get("mode", "driving")

                origin_geo = amap_geocode(origin_name, city)
                destination_geo = amap_geocode(destination_name, city)
                ...
                resp = requests.get(
                    f"https://restapi.amap.com{mode_to_path.get(mode, '/v3/direction/driving')}",
                    params={
            ...
            return {
                "ok": False,
                "degraded": True,
                "message": f"{action_name} 直连HTTP失败，已降级: {exc}",
